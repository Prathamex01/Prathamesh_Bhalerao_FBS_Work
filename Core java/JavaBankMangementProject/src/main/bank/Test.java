...
import account.type.CurrentAccount;
import account.type.LoanAccount;
import account.type.SalaryAccount;
import account.type.SavingAccount;
...
//                continue;
    ...
    static void seedDemoData() {
     ...
     static void applyInterest() {
        for (BankAccount acc : accounts) {
            if (acc instanceof account.type.SavingAccount sa) {
                double interest = sa.getBalance() * account.type.SavingAccount.getSavingInterestRate() / 100;
                sa.deposit(interest);
                System.out.println("Applied interest " + interest + " to account " + sa.getAccountNumber());
            } else if (acc instanceof account.type.SalaryAccount sal) {
                double interest = sal.getBalance() * 4.5 / 100; // example rate
     ...
     static void repayLoan() {
        BankAccount acc = selectAccount();
        if (acc instanceof account.type.LoanAccount loanAcc) {
